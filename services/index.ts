...
// Buscar la interfaz Reserva y agregar el campo reserva
...
export const usuarioService = {
  ...
  update: async (id: number, usuario: Partial<Usuario>): Promise<Usuario> => {
    ...
    // Convertir nombres de propiedades a PascalCase para el backend
    const usuarioToSend = {
      ID_Usuario: id,
...
export const reservaService = {
  ...
  create: async (reserva: Omit<Reserva, "iD_Reserva" | "fechaCreacion">): Promise<Reserva> => {
    // Asegurarse de que tiempoTolerancia tenga el formato correcto
    let tiempoTolerancia = reserva.tiempoTolerancia

    // Si no tiene formato de TimeSpan, convertirlo
    if (!/^\d{2}:\d{2}:\d{2}$/.test(tiempoTolerancia)) {
      ...
      if (numeros && numeros.length > 0) {
        ...
        tiempoTolerancia = `${horas.toString().padStart(2, "0")}:${minutosRestantes.toString().padStart(2, "0")}:00`
      } else {
